defmodule ToyRobot.Simulation do
  ...
  def place(table, placement) do
    if table |> Table.valid_position?(placement) do
      {:ok, %__MODULE__{table: table, robot: struct(Robot, placement)}}
    else
      {:error, :invalid_placement}
    end
  end
  ...
  A valid movement:
      ...
      iex> table = %Table{north_boundary: 4, east_boundary: 4}
      %Table{north_boundary: 4, east_boundary: 4}
      iex> simulation = %Simulation{table: table, robot: %Robot{north: 1, east: 0, facing: :north}}
      iex> simulation |> Simulation.move
      {:ok, %Simulation{
        table: table,
        robot: %Robot{north: 2, east: 0, facing: :north}
      }}
  ...
  def move(%{robot: robot, table: table} = simulation) do
    ...
    valid_movement = table |> Table.valid_position?(moved_robot)
